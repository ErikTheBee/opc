...
static UA_StatusCode readNodeId(UA_NodeId childId, UA_Boolean isInverse, UA_NodeId referenceTypeId, void *handle){
    if(isInverse)
        return UA_STATUSCODE_GOOD;
    readNodeIdReturn* rNIR = (readNodeIdReturn*)handle;
    UA_QualifiedName browseName;
    UA_Server_readBrowseName(rNIR->server,childId,&browseName);
    if (UA_String_equal(&browseName.name,&rNIR->browseName.name) && rNIR->found==false){
        rNIR->found = true;
        rNIR->nodeId = childId;
    }
    return UA_STATUSCODE_GOOD;
...
static void createMessageObject(UA_Server *server,UA_NodeId parentId,UA_String *Sender,UA_String *Receiver,UA_String *Message){
    ...
    UA_Variant_setScalarCopy(&receiverValue,Receiver, &UA_TYPES[UA_TYPES_STRING]);

    UA_Variant messageValue;
    UA_Variant_init(&messageValue);
    UA_Variant_setScalarCopy(&messageValue,Message, &UA_TYPES[UA_TYPES_STRING]);


    //create message object and initialize its values
    UA_NodeId newNodeId;
    UA_Server_addObjectNode(server, getNewNodeId(1), parentId ,UA_NODEID_NUMERIC(0,UA_NS0ID_ORGANIZES),UA_QUALIFIEDNAME(0,"message"),UA_NODEID_NUMERIC(0, 58),objAtr,NULL,&newNodeId);
    UA_NodeId parNodeId = newNodeId;

    UA_Server_addVariableNode(server, getNewNodeId(1), parNodeId ,UA_NODEID_NUMERIC(0,UA_NS0ID_HASCOMPONENT), UA_QUALIFIEDNAME(0,"Sender"), UA_NODEID_NULL, varAtr1, NULL, &newNodeId);
    UA_Server_writeValue(server,newNodeId,senderValue);

    UA_Server_addVariableNode(server, getNewNodeId(1), parNodeId ,UA_NODEID_NUMERIC(0,UA_NS0ID_HASCOMPONENT), UA_QUALIFIEDNAME(0,"Receiver"), UA_NODEID_NULL, varAtr2, NULL, &newNodeId);
    UA_Server_writeValue(server,newNodeId,receiverValue);

    UA_Server_addVariableNode(server, getNewNodeId(1), parNodeId ,UA_NODEID_NUMERIC(0,UA_NS0ID_HASCOMPONENT), UA_QUALIFIEDNAME(0,"Message"), UA_NODEID_NULL, varAtr3, NULL, &newNodeId);
...
static UA_StatusCode dropMessageFunction(void *handle, const UA_NodeId objectId,
    ...
    UA_Server_forEachChildNodeCall(server,componentFolderNodeId,&readNodeId,&rNIR);
    if(rNIR.found){
        //get inbox nodeId
        ...
        rNIR1.found = false;
        rNIR1.server =server;
        UA_Server_forEachChildNodeCall(server,rNIR.nodeId,&readNodeId,&rNIR1);
        if(rNIR1.found){
            createMessageObject(server,rNIR1.nodeId,sender,receiver,message);
        }
...
static void createComponent(UA_ObjectAttributes* objAtrC1, UA_NodeId newNodeId,
    ...
    UA_NodeId inboxNodeId = UA_NODEID_NUMERIC(nodeId.namespaceIndex,nodeId.identifier.numeric+1);
    UA_Server_addObjectNode(server, getNewNodeId(nodeId.namespaceIndex), parentNodeId,
            UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), browseNameInbox,
            UA_NODEID_NUMERIC(0, 61), objAtrInbox, NULL, &inboxNodeId);
}
...
int main(void) {
    ...
    app2.applicationName.text = UA_STRING_ALLOC("Component1");
    ...
    ns[1] = 1;
    ns[2] = 2;
    UA_Server_addApplication(server,&app2,ns,3);

    UA_ApplicationDescription app3;
    UA_ApplicationDescription_copy(&config.applicationDescription, &app3);
    UA_String_deleteMembers(&app3.applicationName.text);
    app3.applicationName.text = UA_STRING_ALLOC("Component2");
    ...
    UA_String_deleteMembers(&app4.applicationName.text);
    app4.applicationName.text = UA_STRING_ALLOC("Component3");
...
*/
        ...
        UA_Argument* inArgs = UA_Array_new(inputArgSize, &UA_TYPES[UA_TYPES_ARGUMENT]);
        ...
        inArgs[0].description = UA_LOCALIZEDTEXT("en_US","Unique receiver address");
        ...
        UA_Server_addMethodNode(server, newNodeId, peerManagerNodeId,
                ...
                dropMsgBrName, dropMessageAtr, &dropMessageFunction,
                (void*) server, inputArgSize, inArgs, 0, NULL, &newNodeId);
