...
class NodeSet(object):
  ...
  def __init__(self):
    self.nodes = {}
    self.namespaces = [u"http://opcfoundation.org/UA/"]
  ...
  def addNodeSet(self, xml, hidden = False):
    # Extract NodeSet DOM
    nodesets = dom.parseString((u'{0}'.format(xml.read()).encode('utf-8'))).getElementsByTagName("UANodeSet")
    ...
    orig_namespaces = extractNamespaces(xml) # List of namespaces used in the xml file
